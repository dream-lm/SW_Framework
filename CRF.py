'''
Author: Zhou Hao
Date: 2022-04-07 17:48:07
LastEditors: Zhou Hao
LastEditTime: 2022-04-07 17:50:29
Description: CRF Complete Random Forest
...
def Crf(train_data, tree_num=100):
...
def Crf_zhou(train_data, tree_num=100):
    """Handling data noise using completely random forest judgment.
    ...
    for _ in range(10):
        ...
        if forest.size == 0:
            ...
            forest = np.hstack((forest, visiTree.reshape(m, 1)))
    orignal_Forest = np.sum(forest, axis=1)     
    nn = 0.5 * tree_num     # Threshold
    
    # Only the noise points need to be removed, keeping the boundary points and internal points
    # 1:true, 0:false
    noiseForest = np.array(list(map(lambda x: 1 if x >= nn else 0, orignal_Forest)))
    weight_denoise = orignal_Forest[np.where(noiseForest == 0)]#

    denoiseTraindata = deleteNoiseData(train_data, noiseForest)
    return denoiseTraindata,weight_denoise  